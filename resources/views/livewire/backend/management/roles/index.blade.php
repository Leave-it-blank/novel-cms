<div>
    <div class="">
        <table class="flex overflow-hidden flex-row mb-5 w-full flex-no-wrap rounded-xs py-3 ">
            ...
            @foreach( $roles = $this->get_roles() as $role)
                <tr x-data="{ open: false , role: {{ $role }} }"
                    ...
                    <td class="py-3 px-4">
                        <div class="inline flex">
                            <div class="mr-2 text-left sm:hidden"> {{'Name: '}}</div> {{$role->name}}
                        </div>
                    </td>
                    <td class="p-3">
                        <div class="inline flex">
                    ...
                    <td class="p-3 truncate">
                        <div class="inline flex">
                            <div
                                class="mr-2 text-left sm:hidden"> {{'Created at: '}}</div> {{$role->updated_at->diffForHumans() }}
                        </div>
                    </td>
                    ...
                    <template x-if="open">
                        <ul @click.away="open = false"
                            ...
                            <li onclick="confirm('Are you sure you want to remove the user from this group?')|| event.stopImmediatePropagation()"
                                x-on:click="@this.delete_role(role)"
