<div class="dark:bg-transparent sm:p-2 xl:p-3 dark:text-black">
    @if($beforeTableSlot)
        <div class="mt-8">
            ...
            <div class="flex-grow h-10 flex items-center">
                ...
                <div class="w-96 flex rounded-lg shadow-sm">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search" class="form-input block bg-gray-50 py-3 focus:bg-white w-full rounded-md pl-10 transition ease-in-out duration-150 sm:text-sm sm:leading-5" placeholder="{{__('Search in')}} {{ $this->searchableColumns()->map->label->join(', ') }}" />
        ...
        <div class="rounded-lg shadow-lg bg-white max-w-screen overflow-x-scroll">
            <div class="rounded-lg @unless($this->hidePagination) rounded-b-none @endif">
                <div class="table align-middle min-w-full">
                    ...
                    <div class="table-row divide-x divide-gray-200">
                        @foreach($this->columns as $index => $column)
                            ...
                            <div class="relative table-cell h-12 w-48 overflow-hidden align-top px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider flex items-center focus:outline-none ">
                                <div class="px-3 py-1 rounded @if(count($selected)) bg-orange-400 @else bg-gray-200 @endif text-black text-center">
                                    {{ count($selected) }}
                                </div>
                            </div>
                            @else
                    ...
                    @empty
                        <div class="p-3 text-lg text-teal-600 flex justify-content-center justify-center">
                            <div>    {{ __("No Books Found!") }} </div>
